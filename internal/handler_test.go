...
func TestJSONAndTextHandlers(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			wantJSON:  `{"source":{"function":"github.com/powerman/slogx/internal.TestJSONAndTextHandlers","file":"handler_test.go","line":$LINE},"msg":"message"}`,
		},
		{
	...
	for _, size := range []int{5, 10, 30, 50, 100} {
		for _, quoting := range []string{"no_quoting", "pre_quoting", "key_quoting", "both_quoting"} {
			b.Run(fmt.Sprintf("%s_prefix_size_%d", quoting, size), func(b *testing.B) {
				var (
					hs = NewTextHandler(io.Discard, nil).next.(*LayoutHandler).
						newHandleState(makeLayoutAttrs(nil), makeLayoutAttrs(nil), buffer.New(), false)
